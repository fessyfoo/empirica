...
| **Lesson** | The transferable distillation of a pattern or anti-pattern — a **cross-practice boundary object** usable by peers, local or remote (unlike a Finding/Decision, which is *local* epistemic state). Also decayable procedural knowledge: an antibody in the cognitive immune system that decays when contradicted by new findings. Carries visibility tiers and propagates via collab because it is built to leave its origin practice. | Decaying + shareable | COLD (YAML) → SEARCH |
...
> **Finding/Decision vs Lesson — the load-bearing distinction.** A Finding or
> Decision is *local* epistemic state: what was learned or chosen in **this**
> practice's context. A **Lesson is the transferable distillation** — a
> pattern/anti-pattern that survives being lifted out of the practice it was
> learned in and applied by another peer, local or remote. It isn't a lesson
> until it crosses the practice boundary and is *actionable on the other side*.
> That's why Lessons alone carry visibility tiers and a propagation path
> (`lesson-create` at `--visibility shared/public` + mesh collab) — findings
> *describe*, lessons *transfer*. (The retired `lesson-replay` was one dead
> mechanism for this; the live mechanism is create-then-propagate.)
...
**Architecture:**
```
VectorRouter (removed in 1.8.14):  vectors → mode (investigate, implement, clarify, ...)
