...
- **`content_poll.poll_and_diff` merges proposals state across polls instead of replacing** (`empirica/core/loop_scheduler/content_poll.py`). Stale-proposal-replay bug surfaced by extension's `prop_e76zksrp7za5tpx3jst2kf2sau` (2026-06-09): ~6 days-old (06-02/03) proposals re-emitted as live wakes across listeners. Reproduced live in empirica's own listener during the triage window. Root cause: the proposals map was rebuilt from scratch every poll; when cortex returned empty for BOTH inbox AND outbox on one cycle (transient empty response — NOT both-failed, which we already guarded), `save_state` wiped the entire `last_seen` map. The next non-empty poll then saw every returned proposal as "new" because `last_seen` was empty, flooding the AI with wake events for proposals it had already acted on (and often already archived). Fix: merge the new entries into the existing `last_seen` instead of replacing. Status changes still emit correctly (`diff_proposals` compares status, not membership). 4 regression tests in `tests/test_loop_content_poll.py` cover two-poll memory, transient-empty preservation, drop-and-return resilience, and status-change sanity. 37 in `test_loop_content_poll.py` + 126 across sibling listener suites green.
- **`mesh diagnose --cortex` ntfy.read_grant probe now authenticates via basic-auth too** (`empirica/cli/command_handlers/_mesh_diagnose_cortex.py` + `mesh_commands.py`). The probe was bearer-only; tenants whose `credentials.yaml` carries `ntfy.user` + `ntfy.password` (basic-auth — listener's actual path) passed `ntfy_token=None`, the probe went no-auth, and ntfy returned 403 → false-negative red flag on the diagnostic. Fix: `_http_head` and `check_ntfy_acl` accept `user`/`password` too; precedence matches the listener's `_ntfy_auth_header` (bearer wins; basic-auth as fallback). `_run_cortex_panel` reads both from the ntfy block and forwards. 4 regression tests cover the basic-auth path, bearer precedence, and the raw urllib header shape. Closes cortex's `prop_m7ns4zq3eva6rpeqcdemifksvu`.
