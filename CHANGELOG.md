...
Substrate work converging the SER tracks (canonical project identity `ser_542199e3` + membrane `ser_4272`) into observable substrate. Six commits add new public surface — a single-verb atomic register, a workspace backfill verb, a daemon source-content endpoint, write-time visibility ladders, session-bootstrap mesh-agreements sync — and close mesh-support's audit `prop_houwq47gu` Break 2 (workspace.db.entity_registry missing project rows).
