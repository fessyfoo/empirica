...
def sample_assessment_response() -> dict[str, Any]:
    ...
    return {
        "engagement": {"score": 0.8, "rationale": "Genuine collaborative intelligence with user"},
        "foundation": {
            "know": {"score": 0.6, "rationale": "Moderate domain knowledge of the task area"},
            "do": {"score": 0.7, "rationale": "Good execution capability for this type of task"},
            "context": {"score": 0.5, "rationale": "Adequate environmental context, but some gaps"},
        },
        "comprehension": {
            "clarity": {"score": 0.8, "rationale": "Clear semantic understanding of requirements"},
            "coherence": {"score": 0.7, "rationale": "Coherent grasp of context and relationships"},
            "signal": {"score": 0.6, "rationale": "Key priority signals identified"},
            "density": {"score": 0.4, "rationale": "Manageable cognitive complexity"},
        },
        "execution": {
            "state": {"score": 0.5, "rationale": "Basic environment state mapping"},
            "change": {"score": 0.6, "rationale": "Can track modification trajectories"},
            "completion": {"score": 0.7, "rationale": "Clear path to completion criteria"},
            "impact": {"score": 0.6, "rationale": "Understand consequence propagation"},
        },
        "uncertainty": {"score": 0.4, "rationale": "Moderate uncertainty about edge cases and unknowns"},
    }
...
def mock_llm_response():
    ...
    def _make_response(vectors: dict[str, float]) -> str:
        ...
        response = {
            "engagement": {"score": vectors.get("engagement", 0.7), "rationale": "Mock engagement rationale"},
            "foundation": {
                "know": {"score": vectors.get("know", 0.5), "rationale": "Mock knowledge rationale"},
                "do": {"score": vectors.get("do", 0.5), "rationale": "Mock capability rationale"},
                "context": {"score": vectors.get("context", 0.5), "rationale": "Mock context rationale"},
            },
            "comprehension": {
                "clarity": {"score": vectors.get("clarity", 0.5), "rationale": "Mock clarity rationale"},
                "coherence": {"score": vectors.get("coherence", 0.5), "rationale": "Mock coherence rationale"},
                "signal": {"score": vectors.get("signal", 0.5), "rationale": "Mock signal rationale"},
                "density": {"score": vectors.get("density", 0.5), "rationale": "Mock density rationale"},
            },
            "execution": {
                "state": {"score": vectors.get("state", 0.5), "rationale": "Mock state rationale"},
                "change": {"score": vectors.get("change", 0.5), "rationale": "Mock change rationale"},
                "completion": {"score": vectors.get("completion", 0.5), "rationale": "Mock completion rationale"},
                "impact": {"score": vectors.get("impact", 0.5), "rationale": "Mock impact rationale"},
            },
            "uncertainty": {"score": vectors.get("uncertainty", 0.5), "rationale": "Mock uncertainty rationale"},
        }
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "integration: Integration tests (deselect with '-m \"not integration\"')")
    config.addinivalue_line("markers", "integrity: Framework integrity tests (core principle validation)")
    config.addinivalue_line("markers", "slow: Slow running tests")
    config.addinivalue_line("markers", "requires_mcp: Tests that require MCP server running")
...
def assert_genuine_assessment():
    ...
    def _assert(assessment_dict: dict[str, Any]):
        ...
        for vector in ["know", "do", "context"]:
            ...
            assert rationale.lower() not in ["adequate", "sufficient", "baseline", "default"], (
