...
def validation_error(message: str, param: str, status_code: int = 400) -> tuple[Any, int]:
    ...
    return jsonify(
        {"ok": False, "error": "validation_error", "message": message, "param": param, "status_code": status_code}
    ), status_code
...
def validate_session_id(session_id: str) -> tuple[Any, int] | None:
    ...
    if len(session_id) > MAX_SESSION_ID_LENGTH:
        return validation_error(f"Session ID too long (max {MAX_SESSION_ID_LENGTH} chars)", "session_id")

    if not (UUID_PATTERN.match(session_id) or SAFE_ID_PATTERN.match(session_id)):
        return validation_error("Invalid session ID format. Use UUID or alphanumeric characters.", "session_id")
...
def validate_ai_id(ai_id: str) -> tuple[Any, int] | None:
    ...
    if len(ai_id) > MAX_AI_ID_LENGTH:
        return validation_error(f"AI ID too long (max {MAX_AI_ID_LENGTH} chars)", "ai_id")

    if not SAFE_ID_PATTERN.match(ai_id):
        return validation_error("Invalid AI ID format. Use alphanumeric, underscore, or hyphen.", "ai_id")
...
def validate_limit(limit: Any, default: int = 20) -> tuple[int, tuple[Any, int] | None]:
    ...
    try:
        ...
        return default, validation_error("Limit must be a positive integer", "limit")

    if limit_int < 1:
        return default, validation_error("Limit must be at least 1", "limit")
...
def validate_offset(offset: Any, default: int = 0) -> tuple[int, tuple[Any, int] | None]:
    ...
    try:
        ...
        return default, validation_error("Offset must be a non-negative integer", "offset")

    if offset_int < 0:
        return default, validation_error("Offset must be non-negative", "offset")
...
def validate_timestamp(timestamp: str | None) -> tuple[Any, int] | None:
    ...
    if len(timestamp) > 64:
        return validation_error("Timestamp too long", "timestamp")

    if not ISO_TIMESTAMP_PATTERN.match(timestamp):
        return validation_error("Invalid timestamp format. Use ISO 8601 (e.g., 2026-02-08T10:00:00Z)", "timestamp")

    return None


def validate_string_length(
    value: str | None, param_name: str, max_length: int = MAX_STRING_LENGTH, required: bool = False
) -> tuple[Any, int] | None:
    """
    ...
    if len(value) > max_length:
        return validation_error(f"{param_name} too long (max {max_length} chars)", param_name)
...
def validate_phase(phase: str | None) -> tuple[Any, int] | None:
    ...
    if phase.upper() not in valid_phases:
        return validation_error(f"Invalid phase. Must be one of: {', '.join(sorted(valid_phases))}", "phase")
    ...
    for name, value in vectors.items():
        if name not in valid_vector_names:
            return validation_error(
                f"Unknown vector: {name}. Valid vectors: {', '.join(sorted(valid_vector_names))}", "vectors"
            )

        try:
            float_val = float(value)
        except (ValueError, TypeError):
            return validation_error(f"Vector '{name}' must be a number", "vectors")

        if not 0.0 <= float_val <= 1.0:
            return validation_error(f"Vector '{name}' must be between 0.0 and 1.0", "vectors")
