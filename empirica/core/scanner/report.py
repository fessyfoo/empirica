...
def _coverage_section(coverage: dict[str, Any]) -> list[str]:
    ...
    out.append(
        f"- **Processes:** {proc.get('succeeded', 0)}/{proc.get('attempted', 0)} ({_percent(proc.get('ratio'))})"
    )
    ...
    out.append(
        f"- **Network sockets:** {net.get('succeeded', 0)}/{net.get('attempted', 0)} ({_percent(net.get('ratio'))})"
    )
...
def _network_section(network: dict[str, Any]) -> list[str]:
    ...
    out.append("")
    out.append(
        f"**Connections seen:** {len(conns)} (of which "
...
def _filesystem_section(filesystem: dict[str, Any]) -> list[str]:
    ...
    for entry in mcp[:20]:
        out.append(
            f"  - `{entry.get('name', '?')}` (command={entry.get('command') or '—'}, args={entry.get('args_count', 0)})"
        )
...
def _env_section(process_env: dict[str, Any], cov: dict[str, Any]) -> list[str]:
    ...
    if not names:
        ...
        out.append("")
        return out
    out.append(f"_Showing {len(names)} of {cov.get('total_env_vars', '?')} total env vars._")
    for name in names:
        out.append(f"  - `{name}`")
    out.append("")
    return out
...
def render_markdown(snapshot: Snapshot) -> str:
    ...
    elapsed = f"{snapshot.finished_at - snapshot.started_at:.2f}s" if snapshot.finished_at else "in progress"
