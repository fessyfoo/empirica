...
def export_to_reflex_logs(
    session_id: str, phase: str, assessment_data: dict[str, Any], log_dir: str = ".empirica_reflex_logs"
) -> Path | None:
    """
    ...
    try:
        ...
        try:
            ...
            if config_path.exists():
                ...
                foundation_weights = config.get(
                    "foundation_confidence_weights", {"know": 0.4, "do": 0.3, "context": 0.3}
                )
            else:
                # Default weights if config file not found
                foundation_weights = {"know": 0.4, "do": 0.3, "context": 0.3}
        except Exception:
        ...
        try:
            comprehension_weights = (
                config.get(
                    ...
                    {"clarity": 0.3, "coherence": 0.3, "signal": 0.2, "density": 0.2},
                )
                if config is not None
                else {"clarity": 0.3, "coherence": 0.3, "signal": 0.2, "density": 0.2}
            )
        except Exception:
            comprehension_weights = {"clarity": 0.3, "coherence": 0.3, "signal": 0.2, "density": 0.2}
        ...
        try:
            execution_weights = (
                config.get(
                    "execution_confidence_weights", {"state": 0.25, "change": 0.25, "completion": 0.25, "impact": 0.25}
                )
                if config is not None
                else {"state": 0.25, "change": 0.25, "completion": 0.25, "impact": 0.25}
            )
        except Exception:
            execution_weights = {"state": 0.25, "change": 0.25, "completion": 0.25, "impact": 0.25}
        ...
        try:
            overall_weights = (
                config.get(
                    ...
                    {"foundation": 0.35, "comprehension": 0.25, "execution": 0.25, "engagement": 0.15},
                )
                if config is not None
                else {"foundation": 0.35, "comprehension": 0.25, "execution": 0.25, "engagement": 0.15}
            )
        except Exception:
            overall_weights = {"foundation": 0.35, "comprehension": 0.25, "execution": 0.25, "engagement": 0.15}
