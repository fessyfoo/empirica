...
def format_notification(n: dict) -> str:
    ...
    return (
        f"**{label}**: {authors} pushed {commit_count} commit(s) touching {len(files)} file(s): {file_list}{msg_hint}"
    )
...
def main():
    ...
    unacked = [
        n
        for n in pending
        if not n.get("acknowledged") and n.get("timestamp_epoch", 0) > cutoff and not n.get("surfaced_in_session")
    ]
