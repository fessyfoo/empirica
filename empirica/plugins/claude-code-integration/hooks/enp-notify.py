...
def main():
    ...
    now = time.time()
    cutoff = now - (48 * 3600)
    unacked = [n for n in pending if not n.get("acknowledged") and n.get("timestamp_epoch", 0) > cutoff]
    ...
    lines.append(
        "*To acknowledge: tell the user about these updates. They will be cleared after 48h or when the user confirms.*"
    )
