...
Examples:
  ...
  empirica query blockers --limit 10         # Goal-linked unknowns (blockers)
    ...
    query_parser.add_argument("--session-id", help="Session ID (required for session scope)")

    query_parser.add_argument("--project-id", help="Project ID (required for project scope)")

    # Common filters
    query_parser.add_argument("--limit", type=int, default=20, help="Maximum results to return (default: 20)")

    query_parser.add_argument(
        "--status", help="Filter by status (type-specific: new/resolved for unknowns, active/completed for goals, etc.)"
    )

    query_parser.add_argument("--ai-id", help="Filter by AI ID")

    query_parser.add_argument("--since", help="Filter by date (ISO format: 2025-01-01)")
