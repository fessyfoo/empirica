...
def add_issue_list_parser(subparsers):
    ...
    parser.add_argument("--session-id", required=False, help="Session ID to list issues for (session-scoped)")

    parser.add_argument(
        "--project-id", required=False, help="Project ID to list issues for (project-scoped, shows all sessions)"
    )

    parser.add_argument(
        "--status", choices=["new", "investigating", "handoff", "resolved", "wontfix"], help="Filter by issue status"
    )
    ...
    parser.add_argument("--severity", choices=["blocker", "high", "medium", "low"], help="Filter by severity level")

    parser.add_argument("--limit", type=int, default=100, help="Maximum number of issues to return (default: 100)")

    parser.add_argument("--output", choices=["json", "human"], default="json", help="Output format (default: json)")
...
def add_issue_show_parser(subparsers):
    ...
    parser.add_argument("--issue-id", required=True, help="Issue ID to show")
...
def add_issue_handoff_parser(subparsers):
    ...
    parser.add_argument("--issue-id", required=True, help="Issue ID to hand off")

    parser.add_argument("--assigned-to", required=True, help="AI ID or name to assign this issue to")
...
def add_issue_resolve_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "issue-resolve", help="Mark issue as resolved", description="Mark an issue as resolved with an explanation"
    )

    parser.add_argument("--session-id", required=True, help="Session ID")

    parser.add_argument("--issue-id", required=True, help="Issue ID that was resolved")

    parser.add_argument("--resolution", required=True, help="How was this issue resolved?")
...
def add_issue_export_parser(subparsers):
    ...
    parser.add_argument("--assigned-to", required=True, help="AI ID to export issues for")
...
def add_issue_stats_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        "issue-stats", help="Show issue capture statistics", description="Display statistics about captured issues"
    )
