...
def add_agent_parsers(subparsers):
    ...
    spawn_parser = subparsers.add_parser(
        "agent-spawn", help="Spawn epistemic agent (returns prompt with branch tracking)"
    )
    ...
    spawn_parser.add_argument(
        "--turtle", action="store_true", help="Auto-select best emerged persona for task (overrides --persona)"
    )
    ...
    # agent-report
    report_parser = subparsers.add_parser("agent-report", help="Report agent postflight results")
    ...
    # agent-aggregate
    aggregate_parser = subparsers.add_parser("agent-aggregate", help="Aggregate results from multiple agents")
    ...
    export_parser = subparsers.add_parser("agent-export", help="Export epistemic agent as shareable JSON package")
    ...
    import_parser = subparsers.add_parser("agent-import", help="Import epistemic agent from JSON package")
    ...
    discover_parser = subparsers.add_parser("agent-discover", help="Discover epistemic agents in sharing network")
    ...
    parallel_parser = subparsers.add_parser(
        "agent-parallel", help="Plan and orchestrate parallel epistemic agents with attention budget"
    )
    ...
    parallel_parser.add_argument("--budget", type=int, default=20, help="Total findings budget (default: 20)")
    parallel_parser.add_argument("--max-agents", type=int, default=5, help="Maximum parallel agents (default: 5)")
    parallel_parser.add_argument(
        "--strategy",
        choices=["information_gain", "uniform", "priority"],
        default="information_gain",
        help="Budget allocation strategy",
    )
    parallel_parser.add_argument(
        "--domains", nargs="+", help="Override investigation domains (auto-detected if not specified)"
    )
