...
def _format_project_banner(project, project_id, db):
    ...
    safe_print(
        f"\U0001f4cd Location: {db.db_path.parent.parent if hasattr(db, 'db_path') and db.db_path else 'Unknown'}"
    )
...
def _format_active_work_section(breadcrumbs):
    ...
    if breadcrumbs.get("active_goals"):
        ...
        for goal in breadcrumbs["active_goals"][:5]:
            beads_link = (
                f" [BEADS: {goal['beads_issue_id']}]" if goal.get("beads_issue_id") else " \u26a0\ufe0f No BEADS link"
            )
...
def _format_memory_gap_section(breadcrumbs):
    ...
    mode_emoji = {"inform": "\U0001f9e0", "warn": "\u26a0\ufe0f", "strict": "\U0001f534", "block": "\U0001f6d1"}.get(
        enforcement, "\U0001f9e0"
    )
...
def _format_memory_gap_details(analysis, breadcrumbs):
    ...
    if gaps_by_type:
        ...
        severity_icon = {"critical": "\U0001f534", "high": "\U0001f7e0", "medium": "\U0001f7e1", "low": "\U0001f535"}
    ...
    try:
        ...
        try:
            ...
            if ws_db_path.exists():
                ...
                ws_cur.execute("SELECT project_type FROM global_projects WHERE id = ?", (project_id,))
