...
def handle_investigate_log_command(args):
    ...
    try:
        ...
        context.setdefault("investigation_log", []).append(
            {"timestamp": datetime.now(timezone.utc).isoformat(), "findings": findings, "evidence": evidence}
        )
        ...
        try:
            ...
            subprocess.run(
                ["git", "notes", "--ref", note_ref, "append", "-m", f"INVESTIGATE: {json.dumps(note_data)}", "HEAD"],
                capture_output=True,
                timeout=5,
            )
...
def handle_act_log_command(args):
    ...
    try:
        ...
        try:
            ...
            subprocess.run(
                ["git", "notes", "--ref", note_ref, "append", "-m", f"ACT: {json.dumps(note_data)}", "HEAD"],
                capture_output=True,
                timeout=5,
            )
        except Exception as e:
