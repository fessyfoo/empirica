...
class EpistemicDocsAgent:
    ...
    def _is_class_ignored(self, class_name: str) -> bool:
        ...
        return any(fnmatch.fnmatch(class_name, pat) for pat in self.config.docs_ignore_classes)
    ...
    def assess_cli_coverage(self, docs_content: str) -> FeatureCoverage:
        ...
        return FeatureCoverage(
            name="CLI Commands", total=len(commands), documented=len(documented), undocumented=sorted(undocumented)
        )
    ...
    def assess_core_coverage(self, docs_content: str) -> tuple[FeatureCoverage, dict[str, list[str]]]:
        ...
        return FeatureCoverage(
            name="Core Modules", total=len(modules), documented=len(documented), undocumented=sorted(undocumented)
        ), category_map
    ...
    def assess_feature_categories(self, docs_content: str, category_map: dict[str, list[str]]) -> list[FeatureCoverage]:
        ...
        for category_name, classes in sorted(category_map.items()):
            ...
            coverage = FeatureCoverage(
                name=category_name, total=len(classes), documented=len(documented), undocumented=sorted(undocumented)
            )
    ...
    def run_assessment(self) -> dict[str, Any]:
        ...
        return {
            ...
            "epistemic_assessment": {"know": know, "uncertainty": uncertainty, "assessment": assessment},
    ...
    def run_turtle_assessment(self, max_rounds: int = 3) -> dict[str, Any]:
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            round_data = {
                ...
                "vectors": {"know": round(know, 2), "uncertainty": round(uncertainty, 2), "delta": round(delta, 3)},
    ...
    def _generate_recommendations(self) -> list[str]:
        ...
        for category in self.categories:
            if category.coverage < 0.50 and category.undocumented:
                recommendations.append(f"Document {category.name}: {', '.join(category.undocumented[:3])}")
    ...
    def check_staleness(
        ...
        if not docs_dir.exists():
            return {"ok": False, "error": f"No docs directory found at {docs_dir}", "issues": []}
        ...
        return {
            ...
            "by_severity": {"high": high[:15], "medium": medium[:15], "low": low[:10]},
            "note": "For deep semantic analysis, use: empirica agent-spawn --task 'Analyze <doc> for staleness'",
        }

    def _check_orphaned_references(self, doc_files: list, docs_dir: Path, cli_commands: set, issues: list) -> None:
    ...
    def _determine_severity(self, score: float, memory_type: str, age_days: int, audience: str) -> str:
        """
    ...
    def _generate_staleness_suggestion(self, memory_type: str, memory_text: str, section_title: str) -> str:
        ...
        return type_suggestions.get(memory_type, f"Review '{section_title}' for potential updates")

    def _generate_notebooklm_suggestions(self) -> dict[str, Any]:
        """Generate NotebookLM content suggestions based on doc structure."""
        docs_dir = self.root / "docs"
        suggestions = {"slide_decks": [], "infographics": [], "audio_overviews": [], "study_guides": []}
        ...
        if "reference" in doc_groups or "reference/api" in doc_groups:
            ...
            suggestions["study_guides"].append(
                {"topic": "CLI & API Reference", "sources": api_docs[:8], "audience": "developer"}
            )
...
def handle_docs_assess(args) -> int:
    ...
    try:
        ...
        if check_staleness:
            result = agent.check_staleness(threshold=staleness_threshold, lookback_days=staleness_days)
        ...
        if summary_only:
            ...
            if output_format == "json":
                ...
                print(
                    f"Docs: {summary['coverage']}% {summary['moon']} | "
...
class DocsExplainAgent:
    ...
    def explain(self, topic: str | None = None, question: str | None = None, audience: str = "all") -> dict[str, Any]:
        ...
        if not docs:
            return {"ok": False, "error": "No documentation found", "explanation": None}
