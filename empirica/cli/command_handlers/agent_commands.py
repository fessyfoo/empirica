        ...
        try:
            ...
            try:
                ...
                if row:
                    ...
                    grounding = {col: row[i] for i, col in enumerate(vector_cols) if row[i] is not None}
                db.close()
            except Exception:
                pass

            turtle_match = sentinel_match_persona(task=task, grounding_vectors=grounding, min_reputation=0.5)
            if turtle_match:
...
def handle_agent_report_command(args) -> dict | int | None:
    ...
    if not postflight_data:
        return {
            "ok": False,
            "error": "Could not parse postflight data. Expected JSON with 'vectors' key or agent output with ```postflight block.",
        }
    ...
    try:
        ...
        if output_format == "json":
            ...
            print(
                f"Merge Score: {merge_score:.4f}" if isinstance(merge_score, (int, float)) else "Merge Score: pending"
            )
...
def handle_agent_aggregate_command(args) -> dict | int | None:
    ...
    try:
        ...
        # Run merge
        merge_result = db.branches.merge_branches(session_id=session_id, investigation_round=investigation_round)
...
def handle_agent_discover_command(args) -> dict | int | None:
    ...
    try:
        ...
        response = {"ok": True, "search_type": search_type, "agents_found": len(agents), "agents": agents}
        ...
        if output_format == "json":
            ...
            for agent in agents:
                ...
                if delta:
                    print(
                        f"      Learning: know {delta.get('know', 0):+.2f}, uncertainty {delta.get('uncertainty', 0):+.2f}"
                    )
                print()
    ...
    try:
        ...
        if output_format == "json":
            ...
            for agent in plan.agents:
                print(f'  empirica agent-spawn --session-id {session_id} --task "{agent.task_focus}" --turtle')
