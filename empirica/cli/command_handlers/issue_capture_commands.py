...
def handle_issue_list_command(args):
    ...
    try:
        ...
        if session_id:
            ...
            issues = service.list_issues(status=status, category=category, severity=severity, limit=limit)
        else:
        ...
        if output == "json":
            result = {
                ...
                "filters": {"status": status, "category": category, "severity": severity},
                "issues": issues,
            }
            ...
            scope_label = (
                ...
                if session_id
                else (f"project {project_id[:8]}..." if project_id else "all projects")
            )
            print(f"\n{'=' * 80}")
            ...
            for issue in issues:
                severity_emoji = {"blocker": "🚫", "high": "⚠️", "medium": "⚠️", "low": "ℹ️"}.get(issue["severity"], "❓")
...
def handle_issue_show_command(args):
    ...
    try:
        ...
        if not session_id or not issue_id:
            result = {"ok": False, "error": "session_id and issue_id are required"}
            print(json.dumps(result))
            return 1
        ...
        if not issue:
            result = {"ok": False, "error": f"Issue {issue_id} not found"}
            print(json.dumps(result))
            return 1
        ...
        if output == "json":
            ...
            try:
                issue["context"] = (
                    json.loads(issue["context"]) if isinstance(issue["context"], str) else issue["context"]
                )
            except Exception:
                pass

            result = {"ok": True, "issue": issue}
            print(json.dumps(result))
        else:
            # Human-readable
            print(f"\n{'=' * 80}")
...
def handle_issue_handoff_command(args):
    ...
    try:
        ...
        if not all([session_id, issue_id, assigned_to]):
            result = {"ok": False, "error": "session_id, issue_id, and assigned_to are required"}
            print(json.dumps(result))
            return 1
        ...
        if success:
            ...
            result = {"ok": False, "error": f"Failed to mark issue {issue_id} for handoff"}
            print(json.dumps(result))
            return 1
...
def handle_issue_resolve_command(args):
    ...
    try:
        ...
        if not all([session_id, issue_id, resolution]):
            result = {"ok": False, "error": "session_id, issue_id, and resolution are required"}
            print(json.dumps(result))
            return 1
        ...
        if success:
            ...
            result = {"ok": False, "error": f"Failed to resolve issue {issue_id}"}
            print(json.dumps(result))
            return 1
...
def handle_issue_export_command(args):
    ...
    try:
        ...
        if not all([session_id, assigned_to]):
            result = {"ok": False, "error": "session_id and assigned_to are required"}
            print(json.dumps(result))
            return 1
        ...
        result = {"ok": True, "export": export_data}
        print(json.dumps(result))
        return 0
...
def handle_issue_stats_command(args):
    ...
    try:
        ...
        if not session_id:
            result = {"ok": False, "error": "session_id is required"}
            print(json.dumps(result))
            return 1
